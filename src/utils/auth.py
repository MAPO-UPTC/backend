    ...
    for role in effective_roles:
        ...
        for entity, actions in role_permissions.items():
            ...
            for action, level in actions.items():
                current_level = all_permissions[entity].get(action, PermissionLevel.NONE.value)
                
                # Jerarquía: ALL > CONDITIONAL > OWN > NONE
                if level == PermissionLevel.ALL.value:
                    all_permissions[entity][action] = level
                elif level == PermissionLevel.CONDITIONAL.value and current_level != PermissionLevel.ALL.value:
                    all_permissions[entity][action] = level
                elif level == PermissionLevel.OWN.value and current_level in [PermissionLevel.NONE.value, PermissionLevel.OWN.value]:
                    all_permissions[entity][action] = level
