...
def get_available_bulk_stock(db: Session, presentation_id: str) -> float:
    ...
    """
    result = (
        db.query(func.sum(BulkConversion.remaining_bulk))
        .filter(
...
def create_sale(db: Session, sale_data: dict, user_id: str) -> Sale:
    ...
    try:
        ...
        customer = (
            db.query(Person).filter(Person.id == sale_data["customer_id"]).first()
        )
        if not customer:
        ...
        for item in sale_data["items"]:
            presentation = (
                ...
                .filter(ProductPresentation.id == item["presentation_id"])
                .first()
            )

            if not presentation:
                raise ValueError(
                    f"Presentación {item['presentation_id']} no encontrada"
                )
        ...
        for item in sale_data["items"]:
            ...
            for lot_detail in lot_details:
                ...
                quantity_from_lot = min(
                    lot_detail.quantity_available, remaining_quantity
                )
            ...
            if remaining_quantity > 0:
                bulk_conversions = (
                    ...
                    .order_by(BulkConversion.conversion_date)
                    .all()
                )
                ...
                for bulk_conv in bulk_conversions:
                    ...
                    quantity_from_bulk = min(
                        bulk_conv.remaining_bulk, remaining_quantity
                    )
...
def get_sales_by_user(
    ...
    """
    return (
        db.query(Sale).filter(Sale.user_id == user_id).offset(skip).limit(limit).all()
    )
    ...
    try:
        ...
        for detail in sale_details:
            ...
            if detail.lot_detail_id:
                lot_detail = (
                    db.query(LotDetail)
                    .filter(LotDetail.id == detail.lot_detail_id)
                    .first()
                )
                if lot_detail:
            ...
            if detail.bulk_conversion_id:
                bulk_conv = (
                    db.query(BulkConversion)
                    .filter(BulkConversion.id == detail.bulk_conversion_id)
                    .first()
                )
                if bulk_conv:
...
def get_best_selling_products(db: Session, limit: int = 10) -> List[dict]:
    ...
    query = (
        ...
        .group_by(SaleDetail.presentation_id)
    ...
    for result in results:
        presentation = (
            db.query(ProductPresentation)
            .filter(ProductPresentation.id == result.presentation_id)
            .first()
        )
        ...
        products.append(
            {
                ...
                "presentation_name": (
                    presentation.presentation_name if presentation else "Unknown"
                ),
...
def get_daily_sales_summary(db: Session, date: datetime) -> dict:
    ...
    total_items = (
        db.query(func.sum(SaleDetail.quantity))
        .join(Sale)
        .filter(Sale.sale_date >= start_of_day, Sale.sale_date <= end_of_day)
        .scalar()
        or 0
...
def get_sale_full_details(db: Session, sale_id: str) -> dict:
    ...
    customer = db.query(Person).filter(Person.id == sale.customer_id).first()

    # Obtener información del vendedor
    seller_user = db.query(User).filter(User.id == sale.user_id).first()

    seller_person = (
        db.query(Person).filter(Person.id == seller_user.person_id).first()
        if seller_user
        else None
    )
    ...
    return {
        ...
        "seller_name": (
            f"{seller_person.name} {seller_person.last_name}"
            if seller_person
            else "Vendedor desconocido"
        ),
