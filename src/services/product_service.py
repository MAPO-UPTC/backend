...
def sell_bulk_service(
    ...
    with Session(engine) as session:
        ...
        if not bulk or bulk.status != "ACTIVE":
            raise HTTPException(
                status_code=404, detail="BulkConversion not found or inactive"
            )
        if bulk.remaining_bulk < quantity:
            raise HTTPException(
                status_code=400, detail="No hay suficiente stock a granel disponible"
            )
...
def get_product_by_id_service(product_id: uuid.UUID):
    ...
    with Session(engine) as session:
        ...
        for presentation in presentations:
            # Calcular stock disponible en lotes normales
            stock_available = (
                session.query(func.coalesce(func.sum(LotDetail.quantity_available), 0))
                .filter(LotDetail.presentation_id == presentation.id)
                .scalar()
                or 0
            )
            ...
            # Calcular stock a granel disponible para esta presentación
            bulk_stock = (
                session.query(func.coalesce(func.sum(BulkConversion.remaining_bulk), 0))
                .filter(
                    BulkConversion.target_presentation_id == presentation.id,
                    BulkConversion.status == "ACTIVE",
                )
