...
async def get_presentation_lot_details(
    ...
    available_only: bool = Query(
        True, description="Filtrar solo lotes con stock disponible"
    ),
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    ...
    try:
        ...
        lot_details = get_lot_details_by_presentation(
            db, presentation_id, available_only=available_only
        )
        ...
        for detail in lot_details:
            ...
            if presentation:
                product = (
                    db.query(Product)
                    .filter(Product.id == presentation.product_id)
                    .first()
                )
            ...
            result.append(
                {
                    ...
                    "received_date": (
                        ...
                        if lot and lot.received_date
                        else None
                    ),
                    "expiry_date": (
                        lot.expiry_date.isoformat() if lot and lot.expiry_date else None
                    ),
                    ...
                    "presentation_name": (
                        presentation.presentation_name if presentation else None
                    ),
        ...
        oldest_date = (
            min(
                lot.received_date
                for lot in [
                    db.query(Lot).filter(Lot.id == d.lot_id).first()
